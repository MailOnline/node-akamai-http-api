...
akamai.getRequestObject = function (path, params, cb, httpHeaders) {
	...
	if (typeof(cb) === 'function') {
		callback = function (err, resp, body) {
			self.logger && self.logger("akamai-http-api:",path,params.headers,err) ;

			// we have an error
			if (err) {
				err.message += "\t"+path+", "+JSON.stringify(params) ;
				return cb(err);
			}
			...
			if (resp.statusCode >= 300) {
				...
				errMsg += "\t"+path+", "+JSON.stringify(params) ;
...
akamai.quickDelete = function (path, cb) {
	this.getRequestObject(path, {
		request: {method: 'put'},
...
/* edited by Szilveszter: you do not need to create the directory */
