...
akamai.create = function(source /*stream*/, cpcode, target, callback) {
	function mkdir(path,idx) {
		return function(done) {
	    	if (idx>path.length)
	    		return done(null) ;
			idx += 1 ;
	    	var l = path.slice(0,idx) ;
	    	akamai.mkdir(l.join("/"),function(err){
    ...
    // Create path
	target = "/"+cpcode+target ;
    var dirPath = target.split("/") ;
    dirPath = dirPath.slice(0,dirPath.length-1).join("/") ;
    
    mkdir(dirPath.split("/"),1)(function(err){
        if (err) {
            err.message = "mkdir "+err.message+"\t"+JSON.stringify(akamai.getConfig()) ;
            return callback && callback(err);
        }
    	akamai.upload(source, target, function(err){
            if (err) {
                err.message = err.message+"\t"+JSON.stringify(akamai.getConfig()) ;
                return callback && callback(err);
            }
            callback && callback();
    	}) ;
